    ...
    const fetchStates = async () => {
      try {
        ...
        if (Array.isArray(response.data)) {
          ...
          console.error("API Error: States response is not an array", response.data);
          setStates([]);
        }
      } catch (error) {
    ...
    const fetchCities = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `${apiUrl}/cities/${formData.state}`
        );
        if (Array.isArray(response.data)) {
          setCities(response.data);
        } else {
          console.error("API Error: Cities response is not an array", response.data);
          setCities([]);
        }
      } catch (error) {
